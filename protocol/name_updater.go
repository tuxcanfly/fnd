...
func NameUpdateBlob(cfg *NameUpdateConfig) error {
	l := nameUpdaterLogger.Sub("name", cfg.Item.Name)
	item := cfg.Item
	defer item.Dispose()
	...
	if header != nil && header.EpochHeight == item.EpochHeight && header.SectorSize >= item.SectorSize {
		return ErrNameUpdaterUpdaterAlreadySynchronized
	}
	...
	if item.EpochHeight < epochHeight {
		return ErrNameUpdaterInvalidEpochBackdated
	}
	...
	if header != nil && item.EpochHeight > epochHeight {
		...
		if !bannedAt.IsZero() {
			...
			if item.EpochHeight <= epochHeight+1 {
				return ErrNameUpdaterInvalidEpochCurrent
			}
		}
		...
		if time.Now().Before(header.EpochStartAt.Add(7 * 24 * time.Duration(time.Hour))) {
			if item.EpochHeight < CurrentEpoch(item.Name)+1 {
				return ErrNameUpdaterInvalidEpochThrottled
			}
		}

		// Reject any epochs more than one in the future
		if item.EpochHeight > CurrentEpoch(item.Name)+1 {
			return ErrNameUpdaterInvalidEpochFuturedated
		}
	...
	defer func() {
		if err := bl.Close(); err != nil {
			nameUpdaterLogger.Error("error closing blob", "err", err)
		}
	}()
	...
	if err != nil {
		if err := tx.Rollback(); err != nil {
			nameUpdaterLogger.Error("error rolling back blob transaction", "err", err)
		}
