...
func TestSealHash(t *testing.T) {
	epochZero := uint16(0)
	sectorSize := uint16(0)
	h := BlobSealHash("testname", epochZero, sectorSize, crypto.ZeroHash, crypto.ZeroHash)
...
func TestSignSeal(t *testing.T) {
	...
	sig, err := BlobSignSeal(crypto.NewSECP256k1Signer(priv), "testname", epochZero, sectorSize, crypto.ZeroHash, crypto.ZeroHash)
